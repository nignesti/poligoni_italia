...
function Header({ navigate }) {
  return (
    <div className="bg-white border-b border-slate-100 px-4 py-3 flex items-center gap-3 sticky top-0 z-40">
      ...
      <h1 className="font-semibold text-slate-900 text-sm">Cronografo</h1>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="px-4 py-10 flex flex-col items-center text-center">
        ...
        <h2 className="text-lg font-bold text-slate-900 mb-1">Cronografo — funzione Pro</h2>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-4 mt-6 w-full max-w-xs">
          <p className="text-xs text-slate-400">Funzione Pro non ancora acquistabile. In arrivo negli abbonamenti Pro.</p>
        </div>
      </div>
    </div>
  );
      ...
      <div className="px-4 py-4 space-y-4">
        <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-4 text-white flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Segnale acustico, rilevamento colpo via microfono</p>
          </div>
        </div>

        <div className="bg-blue-50 border border-blue-200 rounded-xl p-3">
          <p className="text-xs text-blue-800 leading-relaxed">{CRONOGRAFO_DISCLAIMER}</p>
        </div>

        {phase === "idle" && (
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 space-y-3">
            <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Ritardo del segnale</h3>
            <div className="flex gap-2">
              <button
                onClick={() => setDelayMode("random")}
                className={`flex-1 py-2 rounded-xl text-sm font-medium transition-colors ${
                  delayMode === "random" ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-600"
                }`}
              >
                Casuale
              </button>
              <button
                ...
                className={`flex-1 py-2 rounded-xl text-sm font-medium transition-colors ${
                  delayMode === "fixed" ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-600"
                }`}
              >
                Fisso
              </button>
            ...
            {delayMode === "fixed" ? (
              <div>
                <label className="text-xs font-medium text-slate-500">Secondi</label>
                <input
                  type="number"
                  min={1}
                  max={20}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-xs font-medium text-slate-500">Min secondi</label>
                  <input
                    type="number"
                    min={1}
                    max={maxDelay}
                ...
                <div>
                  <label className="text-xs font-medium text-slate-500">Max secondi</label>
                  <input
                    type="number"
                    min={minDelay}
                    max={20}
        ...
        {phase === "arming" && (
          <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100 flex flex-col items-center text-center">
            <div className="w-14 h-14 rounded-full bg-slate-100 flex items-center justify-center mb-3 animate-pulse">
              <Mic className="w-6 h-6 text-slate-500" />
            </div>
            <p className="text-sm font-semibold text-slate-900">Preparati…</p>
            <p className="text-xs text-slate-400 mt-1">Il segnale partirà a breve</p>
          </div>
        )}
        ...
        {phase === "listening" && (
          <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100 flex flex-col items-center text-center">
            ...
            <p className="text-sm font-semibold text-slate-900">In ascolto…</p>
            <p className="text-xs text-slate-400 mt-1">{shots.length} colpo/i rilevato/i</p>
            <button
              onClick={handleStop}
        ...
        {phase === "error" && (
          <div className="bg-red-50 border border-red-200 rounded-2xl p-4 text-center">
            ...
            <button
              onClick={handleReset}
              className="mt-3 flex items-center justify-center gap-2 mx-auto bg-white border border-red-200 text-red-700 font-medium py-2 px-4 rounded-xl text-sm"
            >
        ...
        {phase === "finished" && (
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
            <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Risultati</h3>
            {shots.length === 0 ? (
              <p className="text-sm text-slate-500 text-center py-4">Nessun colpo rilevato</p>
            ) : (
              <div className="space-y-2">
                {shots.map((t, i) => (
                  <div key={i} className="flex items-center justify-between bg-slate-50 rounded-xl px-3 py-2.5">
                    <span className="text-xs text-slate-500">{i === 0 ? "Tempo di reazione" : `Colpo ${i + 1} — split`}</span>
                    <span className="text-sm font-bold text-slate-900">{formatMs(splits[i])}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {history.length > 0 && (
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <h2 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Cronologia</h2>
            </div>
            <div className="space-y-2">
              {history.slice(0, 10).map((run) => (
                <div key={run.id} className="bg-white rounded-xl p-3 shadow-sm border border-slate-100 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-slate-900">
                      {run.shots.length} colpo/i · reazione {formatMs(run.shots[0])}
                    </p>
                    <p className="text-xs text-slate-400">{new Date(run.recorded_at).toLocaleString("it-IT")}</p>
                  </div>
                </div>
              ))}
            </div>
